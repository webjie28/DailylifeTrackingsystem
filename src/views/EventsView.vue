<template>
 <div class="planner-page">
  ...
  <div class="planner-layout"><section class="planner-calendar" aria-label="Calendar"><div class="planner-month"><div><h2 aria-live="polite">{{ calendarTitle }}</h2><span>{{ monthEvents }} {{ monthEvents === 1 ? 'event' : 'events' }} planned in this view</span></div><div class="planner-controls"><label class="planner-view-picker">View <select v-model="calendarView" aria-label="Calendar view"><option value="month">Month</option><option value="custom">Custom</option></select></label><button @click="goToday">Today</button><button v-if="calendarView === 'month'" @click="changeMonth(-1)" aria-label="Previous month">‹</button><button v-if="calendarView === 'month'" @click="changeMonth(1)" aria-label="Next month">›</button></div></div><div v-if="calendarView === 'custom'" class="planner-custom-month"><label for="planner-custom-month">Select month and year</label><input id="planner-custom-month" type="month" v-model="customMonth"></div><div class="planner-weekdays" aria-hidden="true"><span v-for="day in ['Mon','Tue','Wed','Thu','Fri','Sat','Sun']" :key="day">{{ day }}</span></div><div class="planner-days"><button v-for="day in calendarDays" :key="day.key" class="planner-day" :class="{muted:!day.current,selected:day.key === selectedDate,today:day.key === store.todayKey}" :aria-pressed="day.key === selectedDate" :aria-label="`${day.key}, ${day.events.length} events`" @click="selectedDate = day.key"><span class="planner-day-number">{{ day.day }}</span><span v-for="event in day.events.slice(0,2)" :key="event.id" class="planner-event-chip" :class="event.category">{{ event.title }}</span><small v-if="day.events.length > 2">+{{ day.events.length - 2 }} more</small></button></div><div class="planner-legend"><span><i></i>Personal</span><span><i class="study"></i>Study / Work</span><span><i class="fitness"></i>Fitness</span><small>Select a day to see your plans</small></div></section>
...
const customMonth = ref(getTodayKey().slice(0, 7))
...
const calendarTitle = computed(() => calendarView.value === 'month' ? monthLabel.value : formatMonthLabel(customMonth.value))
...
const monthEvents = computed(() => calendarEvents.value.filter(e => calendarDays.value.some(day => day.key === e.date)).length)
const calendarDays = computed(() => {
  if (calendarView.value === 'custom') {
    const selectedMonth = new Date(`${customMonth.value}-01T00:00:00`)
    if (Number.isNaN(selectedMonth.getTime())) return []
    const start = new Date(selectedMonth)
    start.setDate(1 - ((start.getDay() + 6) % 7))
    return Array.from({length:42}, (_, index) => {
      const date = new Date(start); date.setDate(start.getDate() + index)
      const key = dateKey(date)
      return { key, day:date.getDate(), current:date.getMonth() === selectedMonth.getMonth(), events:calendarEvents.value.filter(e => e.date === key) }
    })
  }
...
function goToday() { const now = new Date(); month.value = new Date(now.getFullYear(),now.getMonth(),1); customMonth.value = dateKey(now).slice(0, 7); selectedDate.value = dateKey(now) }
function formatMonthLabel(value) { const date = new Date(`${value}-01T00:00:00`); return Number.isNaN(date.getTime()) ? 'Choose a month' : date.toLocaleDateString('en-US', { month: 'long', year: 'numeric' }) }
...
.planner-view-picker { display:flex; align-items:center; gap:5px; color:var(--cal-muted); font-size:10px; }.planner-view-picker select,.planner-custom-month input { border:1px solid var(--cal-line); border-radius:7px; background:var(--cal-bg); color:var(--cal-ink); padding:7px 8px; font-size:11px; }.planner-custom-month { display:flex; align-items:center; gap:8px; padding:0 24px 18px; color:var(--cal-muted); font-size:10px; }
