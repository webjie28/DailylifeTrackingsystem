<template>
 <div class="planner-page">
  ...
  <section class="planner-schedule"><div class="planner-schedule-top"><h2>Your connected schedule</h2><div role="group" aria-label="Filter schedule"><button v-for="tab in ['all','upcoming','passed']" :key="tab" :aria-pressed="activeTab === tab" @click="activeTab = tab">{{ tab === 'passed' ? 'Past' : tab }}</button></div></div><p v-if="!filteredEvents.length" class="planner-list-empty">No events in this view. Your plans will appear here as you add them.</p><article v-for="event in filteredEvents" :key="event.id" class="planner-list-row"><span class="planner-date-badge"><small>{{ getMonthAbbr(event.date) }}</small><strong>{{ getDayNumber(event.date) }}</strong></span><div><h3>{{ event.title }}</h3><p>{{ formatTimeLabel(event.time) }} · {{ event.category }}{{ event.system ? ' · Synced from tracker' : '' }}</p></div><button v-if="!event.system" @click="startEdit(event)" :aria-label="'Edit scheduled event: ' + event.title">Edit ↗</button></article></section>
